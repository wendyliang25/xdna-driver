...
// reading it.  A no-op where the mapping is coherent.
static void
invalidate_cmd(const cmd_buffer *boh)
{
  auto sz = boh->get_properties().size;
#if defined(__aarch64__)
  // Userspace DC CIVAC may be disallowed at EL0 (SCTLR_EL1.UCI), so have the
  // driver invalidate via SYNC_BO instead of a userspace cache op.
  const_cast<cmd_buffer *>(boh)->sync_by_driver(
