 ...
 * amdxdna's own reserved-memory -- a reusable "shared-dma-pool" region bound to
 * a per-bank child device and allocated with dma_alloc_noncoherent() -- or
...
/*
 * One memory bank. @dev is the device buffers are dma_alloc_noncoherent()'d
 * through: for a bank that names a reserved shared-dma-pool it is a child device
 * we create and bind that pool to (@owns_dev); otherwise it is this device
 * (system CMA).
 */
struct amdxdna_mem_bank {
	struct device	*dev;
	bool		fw;		/* firmware-visible (id AMDXDNA_MEM_BANK_FW) */
	bool		owns_dev;	/* true: @dev is our child, destroy on free */
};
					   ...
					   enum dma_data_direction direction)
{
	...
	n_entries = (cmabuf->size + max_seg - 1) / max_seg;
	sgl = kzalloc_objs(*sg, n_entries);
	if (!sgl) {
		ret = -ENOMEM;
		goto free_sgt;
...
static struct dma_buf *amdxdna_cmabuf_alloc(struct amdxdna_dev *xdna,
					    struct device *dev, size_t size, u64 align)
{
	DEFINE_DMA_BUF_EXPORT_INFO(exp_info);
	 ...
	 * satisfies @align (e.g. the dev-heap's self-alignment); alignments beyond
...
{
	return amdxdna_cmabuf_alloc(xdna, bank->dev, size, align);
}
				 ...
				 u64 alignment, u64 flags)
{
	...
	return amdxdna_cmabuf_alloc(xdna, dev->dev, size, alignment);
}
			  ...
			  void **vaddr, dma_addr_t *dma_addr)
{
	...
	if (bank)
		ret = amdxdna_kbuf_cma(kbuf, bank->dev, size, dir);
...
static void amdxdna_pool_dev_release(struct device *dev)
{
	kfree(dev);
}

/*
 * A bank that names a reserved shared-dma-pool gets its own child device with
 * that pool bound as its CMA area, so dma_alloc_noncoherent(dev) pulls cacheable
 * pages from it. We must not bind the pool to this->ddev.dev (that would hijack
 * its default allocator), hence a dedicated child. No IOMMU is involved (banks
 * are the non-iova path), so a plain dma_direct device with a permissive mask is
 * enough; the firmware bank's reach comes from the region being placed below
 * 4 GB, not from this mask.
 */
static struct device *amdxdna_pool_dev_create(struct amdxdna_dev *xdna,
					      struct device_node *np, int idx, u32 id)
{
	struct device *dev;
	int ret;
	...
	dev->release = amdxdna_pool_dev_release;
	dev->coherent_dma_mask = DMA_BIT_MASK(64);
	dev->dma_mask = &dev->coherent_dma_mask;
	dev_set_name(dev, "%s-bank%u", dev_name(xdna->ddev.dev), id);
...
static void amdxdna_pool_dev_destroy(struct device *dev)
{
...
/*
 * Create a bank. When @np names a reserved shared-dma-pool at memory-region
 * index @region_idx the bank owns a child device bound to it; pass
 * @region_idx < 0 (or a node without that region) to back the bank with system
 * CMA on this device instead.
 */
static struct amdxdna_mem_bank *
amdxdna_bank_create(struct amdxdna_dev *xdna, struct device_node *np,
		    int region_idx, u32 id, bool fw)
{
	...
	if (rmem_np) {
		of_node_put(rmem_np);
		bank->dev = amdxdna_pool_dev_create(xdna, np, region_idx, id);
		if (IS_ERR(bank->dev)) {
			int ret = PTR_ERR(bank->dev);

			kfree(bank);
			return ERR_PTR(ret);
		}
		bank->owns_dev = true;
		XDNA_INFO(xdna, "%s bank %u: shared-dma-pool on %s",
			  fw ? "fw" : "aie", id, dev_name(bank->dev));
	} else {
		bank->dev = xdna->ddev.dev;
		XDNA_INFO(xdna, "%s bank %u: system CMA on %s",
			  fw ? "fw" : "aie", id, dev_name(bank->dev));
	}
...
{
	if (bank->owns_dev)
		amdxdna_pool_dev_destroy(bank->dev);
	kfree(bank);
}
...
static int amdxdna_bank_ensure(struct amdxdna_dev *xdna, u32 id, bool fw)
{
	...
	bank = amdxdna_bank_create(xdna, NULL, -1, id, fw);
	if (IS_ERR(bank))
		return PTR_ERR(bank);
...
int amdxdna_mem_banks_init(struct amdxdna_dev *xdna, struct device_node *np)
{
	...
	for (i = 0; i < count; i++) {
		...
		bank = amdxdna_bank_create(xdna, np, i, id, fw);
		if (IS_ERR(bank)) {
			ret = PTR_ERR(bank);
			goto err;
		}
