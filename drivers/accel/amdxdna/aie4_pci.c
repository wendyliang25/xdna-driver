...
/* Transport hooks for the shared aie4 lifecycle (aie4.c); PCI implementations. */
int aie4_dev_setup(struct amdxdna_dev *xdna)
{
	return aie4m_pcidev_init(xdna);
}

int aie4_hw_start_quirk(struct amdxdna_dev_hdl *ndev)
{
#ifdef AMDXDNA_NPU3A
	return aie4_iommu_bypass_echo(ndev);
#else
	return 0;
#endif
}
...
{
	pci_disable_device(to_pci_dev(xdna->ddev.dev));
}
