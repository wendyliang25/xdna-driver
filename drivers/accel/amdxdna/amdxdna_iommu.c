...
int amdxdna_iommu_init(struct amdxdna_dev *xdna)
{
...
#ifdef HAVE_xen_phy_dma_ops
	XDNA_WARN(xdna, "PASID paging domain alloc dev=%s iommu_group_id=%d returned %ld (%s)",
		  dev_name(xdna->ddev.dev), iommu_group_id(xdna->group),
		  PTR_ERR(xdna->domain),
		  IS_ERR(xdna->domain) ? "err" :
		  (xdna->domain ? "ok" : "null"));
	/*
	...
	if (PTR_ERR(xdna->domain) == -EOPNOTSUPP) {
		XDNA_WARN(xdna, "PASID unsupported, retrying plain paging domain alloc");
		xdna->domain = iommu_paging_domain_alloc(xdna->ddev.dev);
		XDNA_WARN(xdna, "plain paging domain alloc returned %ld (%s)",
			  PTR_ERR(xdna->domain),
			  IS_ERR(xdna->domain) ? "err" :
			  (xdna->domain ? "ok" : "null"));
	}
	...
	if (IS_ERR_OR_NULL(xdna->domain)) {
		XDNA_ERR(xdna, "Failed to alloc iommu domain, ret %ld",
			 PTR_ERR(xdna->domain));
