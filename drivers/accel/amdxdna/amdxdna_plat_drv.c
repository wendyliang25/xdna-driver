...
/*
 * The device tree names the part as "amd,xdna-<hex-id>" (specific, carrying the
 * device id for userspace) plus the generic "amd,xdna".  The driver is
 * id-agnostic, so it binds on the generic string; userspace reads the id from
 * the specific one.
 */
