 ...
 */
static inline void aie4_sync_cmd_for_device(struct amdxdna_gem_obj *cmd_abo)
{
			  ...
			  bool first_cmd, u64 *seq)
{
	 ...
	 */
	aie4_sync_cmd_for_device(cmd_abo);
...
static void job_abort(struct amdxdna_sched_job *job)
{
	...
	aie4_sync_cmd_for_device(job->cmd_bo);
	/*
					...
					struct amdxdna_gem_obj *cmd_abo)
{
	...
	if (sub_abo) {
		...
		aie4_sync_cmd_for_device(sub_abo);
	}
	mutex_unlock(&priv->io_lock);
