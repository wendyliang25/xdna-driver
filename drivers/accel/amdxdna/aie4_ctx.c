 ...
 */
static void sync_umq_range(struct amdxdna_hwctx_priv *priv, const void *p,
 ...
 */
static void sync_cmd_for_device(struct amdxdna_gem_obj *cmd_abo)
{
	amdxdna_gem_dma_sync_range(cmd_abo, 0, cmd_abo->mem.size, DMA_TO_DEVICE);
}
...
static u64 get_read_index(struct amdxdna_hwctx *hwctx)
{
	...
	sync_umq_range(priv, priv->umq_read_index, sizeof(*priv->umq_read_index),
		       DMA_FROM_DEVICE);
	...
	if (!valid_queue_index(ri, wi, CTX_MAX_CMDS)) {
		sync_umq_range(priv, priv->umq_read_index,
...
static u64 publish_cmd(struct amdxdna_hwctx *hwctx)
{
	...
	sync_umq_range(priv, priv->umq_write_index, sizeof(*priv->umq_write_index),
		       DMA_TO_DEVICE);
	return wi;
}
			     ...
			     u16 entries)
{
	...
	for (i = 0; i < entries; i++, dpu++, hipe++) {
		...
		sync_umq_range(priv, hipd, sizeof(*hipd), DMA_TO_DEVICE);
	}
...
static void job_abort(struct amdxdna_sched_job *job)
{
	...
	sync_cmd_for_device(job->cmd_bo);
	/*
