				     ...
				     int (*cpy)(void *to, const void *from, size_t n))
{
	...
	if (start + req_size > log_size) {
		...
		amdxdna_msg_buff_sync(hdl, start, first, DMA_FROM_DEVICE);
		if (cpy(buf, to_cpu_addr(hdl, start), first))
			return -EFAULT;

		/* Wrap-around chunk: from 0 to the remainder */
		amdxdna_msg_buff_sync(hdl, 0, req_size - first, DMA_FROM_DEVICE);
		if (cpy(buf + first, to_cpu_addr(hdl, 0), req_size - first))
			return -EFAULT;
	} else {
		amdxdna_msg_buff_sync(hdl, start, req_size, DMA_FROM_DEVICE);
...
static bool amdxdna_dpt_update_tail(struct amdxdna_dpt *dpt)
{
	...
	amdxdna_msg_buff_sync(dpt->buf, offset, sizeof(*footer), DMA_FROM_DEVICE);
